...
void bench_sdpa_primitives(engine::kind ekind, memory::data_type dt,
    ...
    const dims q_sz = {p.mb, p.head_num, p.query_num, p.head_size};
    const dims k_sz = {p.mb, p.head_num, p.head_size, p.seq_len};
    const dims v_sz = {p.mb, p.head_num, p.seq_len, p.head_size};
    const dims score_sz = {p.mb, p.head_num, p.query_num, p.seq_len};
    const dims scale_sz = {1, 1, 1, 1};
